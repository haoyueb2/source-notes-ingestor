...
- Freeze architecture boundaries, interfaces, library layout, and source strategy.
...
- Implement library writing, asset download handling, and state persistence.
- Support first-run full sync and repeat incremental sync.

Exit criteria:
- A target Zhihu author yields answers, thoughts, and articles in the library.
...
## v2 agent tooling
- Provide a repo-local search skill/plugin instead of a Python QA runtime.
- Standardize an agent retrieval flow around `rg` and direct note reads.
- Validate that agents can answer against the note library without touching crawler internals.

Exit criteria:
- The repo-local plugin/skill can be loaded.
- Agent workflows can answer source-specific questions using raw library notes through shell search and direct note reads.
...
- Add backup and recovery guidance for state and library writes.
...
- The note library remains the single agent-facing source of truth.
